...
void main() {
  ...
  //   Phase1: treatTarget=floor(18×0.8451)=15, controlTarget=floor(18×0.1549)=2
  ...
  group('Phase transition', () {
    test(
      'Phase 1 → Phase 2 when treatment and control targets are met',
      () async {
        final store = makeStore();
        // Fill Phase 1 Female: 15 Treatment + 2 Control → Phase 1 complete
        _fillRecords(store, n: 15, assignment: 'Treatment', phase: 'Phase1');
        _fillRecords(store, n: 2, assignment: 'Control', phase: 'Phase1');
    ...
    test(
      'Phase 1 continues when only treatment target met but not control',
      () async {
        final store = makeStore();
        _fillRecords(store, n: 15, assignment: 'Treatment', phase: 'Phase1');
        _fillRecords(
    ...
    test(
      'Phase 1 continues when only control target met but not treatment',
      () async {
        final store = makeStore();
        _fillRecords(
          store,
    ...
    test('male and female phases transition independently', () async {
      ...
      // male Phase1 targets: treatTarget=floor(12×0.8451)=10, controlTarget=floor(12×0.1549)=1
      ...
      expect(
        male.recruitmentPhase,
        'Phase2',
        reason: 'Male should be in Phase 2',
      );

      final female = TestParishes.eligibleFemale();
      await engine.processEnrollment(female);
      expect(
        female.recruitmentPhase,
        'Phase1',
        reason: 'Female should still be in Phase 1',
      );
    });
  });
}
